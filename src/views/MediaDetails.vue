<template>
  <main class="will-change-scroll">
    ...
    <!-- ===== المحتوى الرئيسي ===== -->
    <div v-else class="media-details-container max-w-7xl mx-auto mt-5 p-5 grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div class="lg:col-span-6">
        ...
        <div class="edit-form-container">
          ...
          <!-- شبكة الحقول الأساسية (من fields) -->
          <div class="form-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-4">
            <div v-for="field in fields" :key="field.key" class="form-group">
              <label :for="`field-${field.key}`"
                class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                {{ field.label }}
              </label>
              <input :id="`field-${field.key}`" v-model="mediaData[field.key]"
            ...
            <div class="form-group">
              <label for="media-category" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                النوع
              </label>
              <select id="media-category" v-model="mediaData.category"
                class="form-control w-full p-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-secondary-dark text-gray-900 dark:text-gray-100">
          ...
          <!-- الحقول المنطقية (booleans) – باستخدام حلقة لتقليل التكرار -->
          <div class="boolean-fields grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div v-for="boolField in booleanFields" :key="boolField.key"
              ...
              <label :for="`bool-${boolField.key}`" class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                {{ boolField.label }}
              </label>
              <input :id="`bool-${boolField.key}`" type="checkbox" v-model="mediaData[boolField.key]"
          ...
          <div class="form-group mb-4">
            <label for="media-id" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
              ID العمل (id)
            </label>
            <input id="media-id" v-model="mediaData.id" readonly
          ...
          <!-- رابط البوستر -->
          <div class="form-group mb-4">
            <label for="poster-url" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
              رابط البوستر
            </label>
            <input id="poster-url" v-model="mediaData.poster_url"
              class="form-control w-full p-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-secondary-dark text-gray-900 dark:text-gray-100">
          </div>

          <!-- قصة العمل (textarea) -->
          <div class="form-group mb-6">
            <label for="media-story" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
              قصة العمل
            </label>
            <textarea id="media-story" v-model="mediaData.story"
              class="form-control w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-secondary-dark text-gray-900 dark:text-gray-100 h-32 max-w-full"></textarea>
          </div>
      ...
      <!-- ===== العمود الأيمن: السيزونات + الحلقات ===== -->
      <div class="lg:col-span-6">

        <!-- ===== قسم السيزونات (للـ series / tv) ===== -->
        <div v-if="mediaData.media_type === 'series' || mediaData.media_type === 'tv'" class="seasons-nav mb-6">
          ...
          <div class="flex gap-2 overflow-x-auto pb-2">
            <button v-for="season in seasons" :key="season.id"
              @click="selectedSeasonId = season.id; loadEpisodes(season.id)" :class="[
                'px-4 py-2 rounded-lg font-bold transition flex items-center gap-2 whitespace-nowrap',
        ...
        <div class="episodes-section">
          <div class="ep-header flex items-center justify-between mb-4">
            ...
            <button @click="addNewEpisodeRow" v-if="['series', 'tv', 'movie'].includes(mediaData.media_type)"
          ...
          <!-- شبكة الحلقات -->
          <div v-if="mediaData.episodes && mediaData.episodes.length"
            class="episodes-grid grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div v-for="ep in mediaData.episodes" :key="ep.id"
              ...
              <div class="flex flex-col">
                <span class="font-bold text-gray-800 dark:text-white text-lg">حلقة {{ ep.episode_number }}</span>
        ...
        <div v-if="showLinksModal"
          class="modal-overlay-sub fixed inset-0 bg-black/50 flex items-center justify-center z-50"
          ...
          <div class="modal-content-sub my-card w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 rounded-xl">
            <h3 class="text-xl font-bold text-gray-800 dark:text-white mb-4">
              إدارة سيرفرات الحلقة: {{ selectedEpisodeId }}
            </h3>

            <div v-for="link in links" :key="link.id" class="link-row flex flex-col sm:flex-row gap-3 mb-3">
              <input v-model="link.server_name" @blur="updateLink(link)" placeholder="اسم السيرفر"
                class="server-name w-full sm:w-1/3 p-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-secondary-dark text-gray-900 dark:text-gray-100">
            ...
            <div
              class="footer--content-sub flex flex-col sm:flex-row items-center justify-between mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 gap-3">
              <button @click="addNewLink(selectedEpisodeId)"
                class="btn-add bg-primary hover:bg-primary-dark text-white font-medium py-2 px-4 rounded-lg transition w-full sm:w-auto">
                إضافة سيرفر جديد
              </button>
              <button @click="showLinksModal = false"
                class="close-btn bg-gray-500 hover:bg-gray-700 text-white font-medium py-2 px-4 rounded-lg transition w-full sm:w-auto">
                إغلاق
              </button>
            </div>
          </div>
